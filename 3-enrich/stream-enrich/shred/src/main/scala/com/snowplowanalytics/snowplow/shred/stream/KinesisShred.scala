package com.snowplowanalytics.snowplow.shred.stream

import java.io.File
import java.net.URI

import com.amazonaws.auth.{AWSCredentials => NoAws, _}
...
import com.snowplowanalytics.snowplow.enrich.stream.{Enrich, SnowplowTracking, utils}
import com.snowplowanalytics.snowplow.enrich.stream.config.{FileConfig, FileConfigOptions}
...
object KinesisShred extends Enrich {
  ...
  override def getSource(
                        ...
                        ): Validation[String, com.snowplowanalytics.snowplow.enrich.stream.sources.Source] =
    KinesisEnrichedSource.createAndInitialize(streamsConfig, resolver, enrichmentRegistry, tracker)
